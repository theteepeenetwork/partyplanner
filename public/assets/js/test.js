...
serviceSelect.id = 'serviceDropdown';
serviceSelect.classList.add('form-control', 'mb-3');

const defaultOption = document.createElement('option');
defaultOption.value = "";
defaultOption.textContent = "Select a service to autofill";
...
// Insert the dropdown before the form
const form = document.getElementById('serviceForm');
form.parentNode.insertBefore(serviceSelect, form);

// Add event listener to autofill the form on selection
serviceSelect.addEventListener('change', (event) => {
    ...
    if (selectedIndex !== "") {
        ...
        // Autofill fields
        document.getElementById('title').value = selectedService.title;
        document.getElementById('short_description').value = selectedService.short_description;
        document.getElementById('description').value = selectedService.description;
        document.getElementById('service_tags').value = selectedService.service_tags;
        document.getElementById('category_id').value = selectedService.category_id;

        // Populate subcategory and third category dynamically
        const subcategoryDropdown = document.getElementById('subcategory_id');
        subcategoryDropdown.innerHTML = '<option value="">Select Subcategory</option>';
        subcategoryDropdown.innerHTML += `<option value="${selectedService.subcategory_id}" selected>Subcategory ${selectedService.subcategory_id}</option>`;

        const thirdCategoryDropdown = document.getElementById('third_category_id');
        thirdCategoryDropdown.innerHTML = '<option value="">Select Further Subcategory</option>';
        thirdCategoryDropdown.innerHTML += `<option value="${selectedService.third_category_id}" selected>Third Category ${selectedService.third_category_id}</option>`;

        activateSubcategoryAndThirdCategory(selectedService.category_id, selectedService.subcategoryId, selectedService.third_category_id);
