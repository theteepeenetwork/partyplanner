...
<?php if (!empty($cmsNavIssues)): ?>
    <div class="alert alert-danger border-0 shadow-sm mb-4" role="alert">
        ...
        <p class="small mb-2">These URLs are linked from the site header or footer. Each needs a <strong>published</strong> row in Admin → Pages.</p>
        <ul class="small mb-3 ps-3">
            <?php foreach ($cmsNavIssues as $issue): ?>
                ...
                <?php else: ?>
                    <li><strong><?= esc($issue['label']) ?></strong> — slug <code><?= esc($issue['slug']) ?></code> is <span class="badge bg-secondary"><?= esc($issue['status'] ?? '') ?></span>. <a href="<?= esc($issue['edit_url'] ?? '') ?>">Edit and publish</a></li>
                <?php endif; ?>
            <?php endforeach; ?>
        </ul>
